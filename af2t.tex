...
% For each $1 \le i \le j\le n+1$, $s[i\mathbin{:}j]:=w_i,w_{i+1},\ldots,w_{j-1}$ is called a \emph{factor} of $s$. (Note the convention that $s[i\mathbin{:}j]$ does not include $w_j$, so $s[i\mathbin{:}j]$ has length $j-i$.)\todo{Is this notation still needed after revisions?}
