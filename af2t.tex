...
% For each $1 \le i \le j\le n+1$, $w[i\mathbin{:}j]:=w_i,w_{i+1},\ldots,w_{j-1}$ is called a \emph{factor} of $w$. (Note the convention that $w[i\mathbin{:}j]$ does not include $w_j$, so $w[i\mathbin{:}j]$ has length $j-i$.)\todo{Is this notation still needed after revisions?}
...
\begin{lem}\label{near_anagram_graph}
    If $\afcn(G_n)\le c$ for each $n\in\N$ then, for each $r_0\in\N$ and $\epsilon>0$, there exists $r\ge r_0$ and a word $w:=x,w_1,x,w_2,\ldots,x,w_{2r}\in\Sigma_0^{4r}$ with $\tau(w)\le\epsilon r$ such that $\phi_w$ is an anagram-free vertex colouring of $G_{n_w}$.
\end{lem}
