...
\begin{thm}\label{lower_bounds}
    ...
    \begin{compactenum}[(a)]
        ...
        \item a graph $G$ of pathwidth $3$ such that any $c$-colouring of $G$ has a repetitive cycle.\label{lower_bound}\todo{I would love to replace cycle by path!}
    \end{compactenum}
\end{thm}
...
\begin{proof}
    ...
    By definition, for every strict subset $X'\subset X$, there exists an integer $n_0:=n_0(X')$, such that any edge $c$-colouring $\varphi$ of $Q_{n_0}$ with $X(\varphi)\subseteq X'$ has a repetitive path. Now let $k:=1+\max\{n_0(X'):X'\subseteq X\}$ and take an edge $c$-colouring $\varphi$ of $Q_k$ with $X(\varphi)\subseteq X$ that has no repetitive path.  Observe that, for any $i\in\{1,\ldots,n-k\}$, $\rho_{i,2}\in X$ and $\varphi$ and $\bigcup_{j=i+1}^{i+k}\rho_{j,2}=X$.  In particular, $\rho_{i,2}=\rho_{j,2}$ for some $j\in\{i+1,\ldots,i+k\}$.
\end{proof}


\begin{lem}\label{cycle_lower_bound}
...
\begin{proof}
    ...
    Without loss of generality we may assume that $P_1$ contains $x_{i_a+1}$ and that \[
        (\varphi(x_1x_2),\varphi(y_1y_2),\varphi(x_1y_2),\varphi(y_1x_2))=(1,2,3,4) \enspace .
    \]
    For each $i\in\{1,\ldots,c\}$ and each $j\in\{1,2\}$, let $n_i(P_j)$ be the number of $e$ of $P_j$ with $\varphi(e)=i$.  There are two cases to consider (see \cref{cases}):
    \begin{figure}
        \begin{center}
    ...
    \end{figure}
    \begin{compactenum}
        \item $P_1$ contains $x_{i_b}$ (and $P_2$ contains $y_{i_b}$):  In this case $n_1(P_1)=n_1(P_2)+1$, $n_2(P_2)=n_2(P_1)+1$, and $n_i(P_1)=n_i(P_2)$ for each $i\in\{3,\ldots,c\}$.  Prepend the path $y_{i_a+1}x_{i_a}x_{i_a+1}$ to $P_1$ to obtain a path $P_1'$.  Append the path $x_{i_b}y_{i_b+1}y_{i_b}$ to $P_2$ to obtain a path $P_2'$.  The union of $P_1'$ and $P_2'$ is a cycle $C$ and $n_i(P_1')=n_i(P_2')$ for each $i\in\{1,\ldots,c\}$. Thus, $C$ is a repetitive cycle.

        \item $P_1$ contains $y_{i_b}$ (and $P_2$ contains $x_{i_b}$): In this case $n_3(P_1)=n_3(P_2)+1$, $n_4(P_2)=n_4(P_1)+1$, and $n_i(P_1)=n_i(P_2)$ for each $i\in\{1,2,4,5,6,\ldots,m-1\}$.  We append the path $y_{i_b}x_{i_b+1}x_{i_b}$ to $P_1$ to obtain a path $P_1'$.  We preprend the path $x_{i_a+1}x_{i_a}y_{i_a+1}$ to $P_2$ to obtain a path $P_2'$.  The union of $P_1'$ and $P_2'$ is a cycle $C$ and $n_i(P_1')=n_i(P_2')$ for each $i\in\{1,\ldots,c\}$.  Thus, $C$ is a repetitive cycle. \qedhere
    \end{compactenum}
\end{proof}
