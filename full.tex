...
For a $2$-tree $G$ and an edge $e\in E(G)$, an $e$-rooted skeleton decomposition $\{(G_x,H_x):x\in V(T)\}$ is a collection of pairs of subgraphs of $G$ indexed by the nodes of a rooted tree $T$ that is defined inductively as follows:
\begin{compactenum}
...
\begin{proof}[Proof of \cref{main} assuming \cref{spine_colouring}]
    ...
    $\{(G_x,H_x): x\in V(T)\}$ be an $e$-rooted hierarchical skeleton decomposition of $G$.  The root $r$ of $T$, define $e_r:=e$. For any node $x$ of $T$ with parent $y$, let $e_x$ be the edge by which $G_x$ attaches to $H_y$.  For each $x\in V(T)$, let $\varphi_x$ a colouring of $\hat{H}_x$ given by \cref{spine_colouring}.

    For $x\in V(T)$, $\depth(x)$ is the depth of $x$ in $T$, i.e., the length of the path from $r$ to $x$. Observe that, for any $k\in\{1,2,3\}$ and any $k$-clique $K$ in $G$, the graph $T[K]:=T[\{x\in V(T):K\in H_x\}]$ is a connected subtree of $T$.  (When $|K|=1$, $T[K]$ is a subtree, when $|K|=2$, $T[K]$ is a vertical path,\footnote{A \emph{vertical path} in a tree $T$ is a path in which no two nodes have the same depth.} and when $|K|=3$, $T[K]$ contains a single vertex.)  We let $\x(K)$ denote the minimum depth node of $T[K]$ and $\depth(K)$ is the depth of $\x(K)$. We abuse notation slightly when $K$ is a $1$ or $2$-clique:  For a vertex $v$ and edge $vw$ we use $\x(v)$ as shorthand for $\x(\{v\})$ and $\x(vw)$ as shorthand for $\x(\{v,w\})$.

    As noted above $T[vw]$ is a vertical path $x_0,\ldots,x_t$ that starts at $x_0=\x(vw)$.  If $t>0$, then we let $G_{vw}:=G_{x_1}$.  If $t=0$, then $G_{vw}$ denote the empty graph.  Thus, $G_{vw}$ attaches to $H_{x_0}$ using the edge $vw$.

    We distinguish between three kinds of edges in $G$: A $c$-edge $e$ is contained in $E(\hat{H}_{\x(e)})$.  A \emph{flat $\Delta$-edge} $e$ is contained in $E(H_{\x(e)})\setminus E(\hat{H}_{\x(e)})$. Any edge of $G$ that is not a $c$-edge or a flat $\Delta$-edge is a \emph{vertical edge}.  Each vertical edge $vw$ has $\depth(v)\neq\depth(w)$. If $\depth(v)<\depth(w)$ then we call $v$ the \emph{upper endpoint} of $vw$ and $w$ is the \emph{lower endpoint} of $vw$.
    ...
    \begin{equation}
      \Delta\ge \deg_G(v_i)\ge j\cdot\deg_{G_{v_iw_j}}(v_i) \enspace ,
    ...
    We now define several additional edge colourings whose product makes up our final edge colouring. First we need a definition.  For any edge $e$ of $G$ and any edge $e'$ of $G_e$, we call $e$ the \emph{projection} of $e'$ onto $H_{\x(e)}$.  Note that, if $e:=vw$ is a vertical edge with $\depth(v)<\depth(w)$, then the projection of $e'$



    Our product colouring has the following four parts:
    \begin{compactenum}[$\varphi_1$:]
        ...
        \item This is a colouring $\varphi_3:E(G)\to\{-\Delta,\ldots,\Delta\}$ that is defined specifically to deal with vertical edges.  For each flat edge $xy$ of $G$, arbitrarily choose one of $x$ or $y$ as the \emph{positive endpoint} of the edge $xy$.  For each vertical edge $vw$ with $\depth(v)<\depth(w)$, let $uv$ be the projection of $vw$ onto $H_{\x(v)}$.  If $v$ is the positive vertex of the edge $uv$ then $\varphi_4(vw):=(\varphi_3(uv),\varphi_1(uv))$.\todo{Reverse roles of $u$ and $w$ here (and later)?} Otherwise, $\varphi_4(vw)=(\varphi_3(uv),-\varphi_1(uv))$.
    \end{compactenum}
    ...
    Suppose that some vertical edge $vw$ with $\depth(v)<\depth(w)$ is coloured so that $\varphi_1(vw)=\ell$ and $|\varphi_4(vw)|=j$. Then $|\varphi_4(vw)|=\varphi_1(uv)$ where $uv$ is the projection of $vw$ onto $H_{\x(v)}$.  If $uv$ is a $c$-edge, then $\varphi_1(uv)\in\{1,\ldots,c\}$, so $(\varphi_1(vw),\varphi_4(vw))\in\{1,\ldots,\Delta\}\times\{-1,1\}\times\{1,\ldots,c\}$, a set of size $2c\Delta$.

    If $uv$ is a flat $\Delta$-edge
    Furthermore, $\varphi_1(vw)=\ell$ implies that $\deg_{G_{uv}}(v)\ge\ell$.  Therefore, by \cref{harmonic_bounder}, $\Delta\ge \deg_G(v)\ge j\ell$.  The number of pairs $j,\ell\in\{1,\ldots,\Delta\}^2$ for which $j\ell\le\Delta$ is at most $\sum_{j=1}^{\Delta}\Delta/j=\Delta H_{\Delta}\le\Delta(1+\ln\Delta)$.

    If $uv$ is a vertical edge.


    This can only happen if $i\le c$ or $vw$ is a flat edge and there are at least $i-1$ edges $vw_1,\ldots,w_{i-1}$ such that the degree of $v$ in $G_{vw_i}$ is at least equal to the degree of $v$ in $G_{vw}$.\footnote{Define $G_{vw}$ notation.}  Since $v$ has degree at most $\Delta$, this implies that $(i-1)|j|\le \Delta$.
    ...
    %
    % But this is not possible since \cref{serpentine_degree4} ensures that the colouring of $H_x$ has no repetitive path.
    %
    % \begin{compactenum}
    %     \item If $k=1$ then $P$ contains the edges $e_0:=v_{i_1-1}v_{i_1}$ and $e_1:=v_{i_1}v_{i_1+1}$.  These are are the only two edges in $P$ of depth $0$.  These two edges have a vertex in common so, since $\varphi_1$ is a proper edge-colouring of $G$\todo{I hope we don't need this}, the colours $\varphi(e_0)$ and $\varphi(e_1)$ are each unique in $P$, so $P$ is certainly non repetitive.
    %
    %     \item If $k\ge 2$, then $P':=v_{i_1},\ldots,v_{i_k}$ is a path in $H_r$.
    %
    %     The edges of $P'$ that are also in $P$ are the only edges $e$ of $P$ for which $\varphi_2(e)=0$ and $\varphi_3(e)=0$.  For every edge $v_{i_j}v_{i_{j+1}}$ of $P'$ not in $P$, $P$ contains the pair of edges $\{e,f\}:=\{v_{i_j}v_{{i_j}+1}, v_{i_{j+1}-1}v_{i_{j+1}}\}$.  Exactly one of these two edges, say $e$, has $\varphi_1(e)=\varphi_1(v_{i_j}v_{i_{j+1}})$.  Additionally, there are two other potential edges $e_{0}:=v_{i_1}-1v_{i_1}$ and $e_{k}:=v_{k}v_{k+1}$ that are in the same category.  All this say that, if $P$ is repetitive, then one of the four paths in $H_x$ determined by $e_0,\ldtos,e_k$, $e_1,\ldots,e_k$, $e_1,\ldots,e_{k-1}$ or $e_0,\ldots,e_{k-1}$ is also repetitive.
    % \end{compactenum}
\end{proof}
