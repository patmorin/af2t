...
%
% Specifically, we require that the rooting of $X$ and $T$ are consistent so that a node $x$ is an $X$-ancestor of a node $y$ if and only if it is $x$ is a $T$-ancestor of $y$.
%
...
% However, we still require a slightly more refined parameter.  For a tree $T$, let $\dc(T)$ be the maximum integer $d$ such that $T$ contains $d$ vertex-disjoint subdivisions of a complete binary tree of height $\sh(T)$.
%
% \begin{lem}\label{dc}
%     For any non-empty rooted tree $T$, $\dc(T)\in\{1,2\}$.
% \end{lem}
%
% \begin{proof}
%     By definition, $T$ contains a subdivision of a completely binary tree of height $\sh(T)$, so $\dc(T)\ge 1$. On the other hand, if $T$ contains three vertex disjoint subdivisions $T_1,T_2,T_3\subseteq T$ with roots $r_1,r_2,r_3\in V(T)$ of a complete binary tree of height $\sh(T)$, then at least one of $r_1r_2$, $r_2r_3$ or $r_3r_1$ is not an edge of $T$.  Without loss of generality, assume $r_1r_2\not\in E(T)$.  Then the path in $T$ from $r_1$ to $r_2$ plus the two subgraphs $T_1$ and $T_2$ are a subdivision of a complete binary tree of height $\sh(T)+1$, contradicting the definition of $\sh(T)$.
% \end{proof}
%
% Finally, we define $b(T):= 2\cdot\sh(T)+\dc(T)-1$. \cref{b_approx,dual_approx,b_approx} imply the following corollary, which allows use to use $b(G^\star)$ as a proxy for $\pw(G)$:
%
% \begin{cor}
%     For any $2$-tree $T$ with $n\ge 3$ vertices, $2b(G^\star)-1\le \pw(G)\le 6b(G^\star)+5$.
% \end{cor}
...
% \begin{proof}
%     Let $T:=G^\star$, $h:=b(G^\star)$ and root $T$ at some arbitrary vertex $r_0$ such that $vw\in r_0$ (recall that the vertices of $T$ are $3$-cliques in $G$).  We will construct a path $P$ beginning at $r_0$ such that $\cbt(T-P)< \cbt(T)$.  If $\cbt(T-\{r_0\}) < \cbt(T)$ then $P:=r_0$ and we are done.  Otherwise, $r_0$ has at most one child $r_1$ such that the subtree $T_{r_1}$ rooted at $r_1$ has $\cbt(r_1)=\cbt(T)$. In this case $P$ is the path that begins with $r_0$ followed by the path obtained by applying this construction recursively from $T_{r_1}$.
%
%     We let $H=G[\bigcup_{x\in V(P)} x]$.  Then, clearly $H$ satisfies (i) and (ii). All that remains is to show that $H$ satisfies (iii). To see why this is the case, consider some edge $vw$ of $H$.  Then $G_{H,vw}$ is attached to $vw$ and $H$ contains a unique $3$-clique $uvw$.  There are two cases to consider:
%
%     \begin{compactenum}
%         \item $\cbt(G_{H,vw}) < \cbt(G)$. In this case, by \cref{dc}, $b(G_{H,vw})=2\cbt(G_{H,vw})+\dc(G_{H,vw})\le 2\cbt(G_{H,vw})+1< 2\cbt(G)$.
%
%         \item $\cbt(G_{H,vw}) = \cbt(G)$.
%         $\dc(G)=1$, so $b(G)=2h$.  In this case, $\cbt(G_{H},vw)\le h-1$
%         \item $\dc(G)=2$, so $b(G)=2h+1$.
%     \end{compactenum}
%
%
%
...
\begin{proof}[Proof of \cref{main} assuming \cref{spine_colouring}]
    ...
    \begin{compactenum}[$\varphi_1$:]
        ...
        \item This is the colouring $\varphi_3:E(G)\to\{-\Delta,\ldots,\Delta\}$ that is defined specifically to deal with vertical edges.  For each flat edge $e=xy$ of $G$, arbitrarily choose one of $x$ or $y$ as designated for $e$.  For each vertical edge $vw$ with $\depth(v)<\depth(w)$, let $uv$ be the projection of $vw$ onto $H_{\x(v)}$.\todo{Move definition of project earlier}.  If $v$ is the designated vertex of the edge $uv$ then $\varphi_1(vw):=(\varphi_3(uv),\varphi_1(uv))$.\todo{Reverse roles of $u$ and $w$ here (and later).} Otherwise, $\varphi_1(vw)=(\varphi_3(uv),-\varphi_1(uv))$.
    \end{compactenum}
